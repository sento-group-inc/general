...
| いま | 第1期15名（Codex研修済み・すでに業務利用が先行して走っている組）に、ガイドライン・環境整備を後追いで当てはめている段階。セキュリティ研修・環境セットアップは新規開始ではなく、稼働と並行して進める |
| 想定期間 | 合意から3ヶ月を目安に、Phase 0〜Phase 2（第1期の定着）まで進める（起点は§9で要確認） |
...
| Phase 2 | 第1期（15名・研修済みですでに稼働中の先行組）へ、ガイドライン適用とセキュリティ研修を並行して当てはめ、後追いで安全な運用に揃える | 実施中 |
...
- **期間**：合意から3ヶ月を目安に進める前提（起点は§9で要確認）
- **予算**：現時点は「制御開発」予算の枠内で進行中。ただし130名への全社展開には全社予算としての位置づけ直しが必要で、現状はこのギャップが未解消
- **デジタル事業部との連携**：現時点では保留。連携の要否・タイミングは別途判断する（§9）
...
- 3ヶ月スパンの起点はいつか（Phase 0合意日か、ガイドライン初版完成日か）
- デジタル事業部との連携を誰が・いつ判断するか
...
3. 第1期15名（研修済み・稼働中の先行組）へのガイドライン適用とセキュリティ研修の実施日程を確定する（環境セットアップの後追い対応は並行して進める）
